...
import { useCreateLostItemRequestMutation } from '../../features/lostFound/lostFoundApiSlice';
import { Upload, ArrowLeft, AlertCircle } from 'lucide-react';
import { toast } from 'react-hot-toast';

const PostLostItemPage = () => {
  const navigate = useNavigate();
  const [createLostRequest, { isLoading }] = useCreateLostItemRequestMutation();
  ...
  const validateForm = () => {
    ...
    if (!formData.landmark.trim()) newErrors.landmark = 'Last seen location is required';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await createLostRequest(formDataToSend).unwrap();
      toast.success('Lost item reported successfully!');
      navigate('/student/lost-found');
    } catch (error) {
      console.error('Failed to report lost item:', error);
      toast.error(error.data?.message || 'Failed to report lost item');
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Brief title for the lost item"
            />
            {errors.title && (
